	...
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
		...
		<nav class="navigation comment-navigation" role="navigation">
			...
			<div class="nav-links d-flex justify-content-between">
				<?php
					if ( $prev_link = get_previous_comments_link( __( '&larr; Older Comments', 'directory-starter' ) ) ) :
						printf( '<div class="nav-previous">%s</div>', $prev_link );
					endif;

					if ( $next_link = get_next_comments_link( __( 'Newer Comments &rarr;', 'directory-starter' ) ) ) :
...
<li <?php comment_class( empty( $args['has_children'] ) ? 'list-unstyled' : 'parent list-unstyled' ); ?> id="comment-<?php comment_ID() ?>">
